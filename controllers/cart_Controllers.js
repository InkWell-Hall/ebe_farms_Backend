...
export const updateCartItem = async (req, res) => {
  try {
    ...
    const cart = await Cart.findById(cartId).populate("items.advert");
    if (!cart) return res.status(404).json({ message: "Cart not found" });

    const itemIndex = cart.items.findIndex(
      (item) => item.advert._id.toString() === advert
    );
...
export const deleteCartItem = async (req, res) => {
  try {
    ...
    if (!itemId) {
      return res.status(400).json({ message: "Item ID is required in the body" });
    }
...
export const getSingleCart = async (req, res) => {
  try {
    ...
    const totalAmount = cart.items.reduce(
      (sum, item) => sum + item.quantity * item.advert.price,
      0
    );
...
export const getAllCarts = async (req, res) => {
  try {
    const carts = await Cart.find()
      .populate('items.advert')
      .populate('user', '-password -otp'); // exclude sensitive fields
...
//       (item) => item.advert.toString() === itemId
//     );
//     if (existingItem) {
//       existingItem.quantity += Number(quantity); // Ensure quantity is a number
//     } else {
//       cart.items.push({ advert: itemId, quantity: Number(quantity) });
//     }
